...
; Copyright 2010, 2012-2023, 2025 Richard Dymond (this disassembly)
;
; ============================================================================
; MANIC MINER - MODERN GAMEPLAY MODIFICATION
; ============================================================================
; Version: Sprint Edition
; Based on: SkoolKit disassembly
;
; ============================================================================
; IMPLEMENTED FEATURES
; ============================================================================
;
; 1. SPRINT MODE (Hold number keys 0-9 or joystick UP)
;    - Willy moves faster when sprint key is held
;    - Speed boost applied every 2nd frame to keep animation visible
;    - Builds slide velocity for momentum when released
;    - Kempston joystick: UP direction triggers sprint
;    Location: SPRINT_CHECK, GROUND_SLIDE routines
;
; 2. SLIDE CONTINUATION (Momentum after releasing sprint)
;    - When sprint is released, Willy continues sliding briefly
;    - Slide velocity decrements each frame until stopped
;    - Direction preserved from last sprint movement
;    Location: GROUND_SLIDE, GS_NO_SPRINT, GS_APPLY_SLIDE
;
; 3. AIR CONTROL (Influence direction while jumping)
;    - No input: continues in jump direction (momentum preserved)
;    - Same direction as jump: boosted movement (extra speed)
;    - Opposite direction: reduced movement (harder to reverse)
;    - Sprint jumps have stronger momentum (harder to turn)
;    - Sprite always faces jump direction (Mario-style, less confusing)
;    Location: AIR_CONTROL routine, JUMP_FACING variable
;
; 4. VARIABLE JUMP HEIGHT (Release jump early for shorter jumps)
;    - Release jump key during rise phase (JUMPING 1-7)
;    - ~2 frames of inertia before descent begins
;    - Creates Mario-style variable jump feel
;    - Mirrors jump arc at cut point for smooth descent
;    Location: VAR_JUMP routine (called from MOVEWILLY)
;
; 5. EXTENDED FALL DISTANCE (Survive longer falls)
;    - Original: AIRBORNE >= 12 = fatal fall
;    - Modified: AIRBORNE >= 18 = fatal fall
;    - Willy can fall ~50% further before dying
;    Location: MOVEWILLY2 (CP 18 instead of CP 12)
;
; 6. EXTRA LIVES (9 lives instead of 3)
;    - Original: LD A,2 (3 lives displayed)
;    - Modified: LD A,8 (9 lives displayed)
;    Location: STARTGAME routine, line ~845
;
; ============================================================================
; NEW VARIABLES (stored after routines, before FREE_SPACE)
; ============================================================================
;
; SPRINT_ACTIVE  - 1 if sprint key held this frame
; SLIDE_VEL      - Ground slide velocity (0-4)
; SLIDE_DIR      - Direction of slide (0=right, 1=left)
; SPRINT_JUMP    - 1 if jump started while sprinting
; SLIDING        - 1 if in passive slide mode
; ANIM_TICK      - Animation tick counter for sprint
; ANIM_FRAME     - Visual animation frame (0-3)
; MOMENTUM_DIR   - Direction at jump start (DMFLAGS format)
; MOMENTUM_CTR   - Counter for boost/reduce timing
; JUMP_FACING    - Sprite facing direction at jump start (0=right, 1=left)
; JUMP_RELEASE   - JUMPING value when jump key released (0=held)
;
; ============================================================================
; BINARY SIZE REQUIREMENT - CRITICAL!
; ============================================================================
;
; The assembled binary MUST be exactly 32771 bytes.
; This fills memory from address 32765 (ORG) to 65535.
;
; To check: pasmo mm_modbase.asm test.bin && wc -c test.bin
; Should output: 32771 test.bin
;
; The FREE_SPACE DEFS acts as padding. When adding code:
;   1. Add your routine ABOVE the FREE_SPACE label
;   2. DECREASE the DEFS value by the number of bytes added
;   3. Reassemble and verify size is still 32771 bytes
;
; Current FREE_SPACE: 1785 bytes remaining
;
; COMMON SIZE ISSUES:
; - If too large: binary overflows, pasmo warns "64KB limit passed"
; - If too small: MESSINTRO moves, game displays garbage on start
; - Always verify MESSINTRO is at address 0x9D00 in symbol file
;
; ============================================================================
; TAP FILE CREATION - CRITICAL!
; ============================================================================
;
; The TAP file must have load address 0x7FFD (32765), matching the ORG.
; Using 0x8000 will cause garbage on screen at startup!
;
; Python TAP creation snippet:
;   header[13:15] = struct.pack('<H', 0x7FFD)  # Load address
;
; ============================================================================
; FEATURES ATTEMPTED BUT NOT IMPLEMENTED
; ============================================================================
;
; HIGHER SPRINT JUMP:
;   Attempted to increase jump height during sprint. Multiple approaches
;   failed due to collision detection issues:
;   - Direct WILLY_Y modification caused floor misalignment
;   - Jump extension caused asymmetric displacement
;   - Floor checks only occur at specific WILLY_Y alignments (mod 16)
;   - Even symmetric changes caused fall-through on landing
;   Conclusion: Jump physics are tightly coupled to collision detection.
;   Changes require deep understanding of WILLY_Y alignment requirements.
;
; FORGIVING ENEMY COLLISION (via KILLWILLY modification):
;   Attempted to add grace period or hitbox reduction at KILLWILLY.
;   Failed due to complex stack state at entry points:
;   - KILLWILLY has multiple entry points (KILLWILLY, KILLWILLY_0, KILLWILLY_1)
;   - Each expects different stack state
;   - RET-based escape corrupted stack
;   - JP doesn't push return address, breaking recovery
;   Conclusion: KILLWILLY modification requires careful stack analysis.
;
; FORGIVING ENEMY COLLISION (via DRWFIX hitbox shrinking):
;   Attempted to mask edge pixels before collision AND operation.
;   Caused garbage display - DRWFIX is used for all sprite drawing,
;   not just collision. Masking affected visual output.
;   Conclusion: Would need separate collision-only path.
;
; ============================================================================
; KEY GAME VARIABLES (for reference)
; ============================================================================
;
; WILLY_Y   (0x8068) - Vertical position (2 * pixel y-coordinate)
; AIRBORNE  (0x806C) - 0=ground, 1=jumping, 2-11=falling safe, 12+=fatal
; JUMPING   (0x806E) - Jump counter 0-17, controls arc
; DMFLAGS   (0x806A) - Bit 0=facing left, Bit 1=moving
; LOCATION  (0x806C) - Attribute buffer address of Willy's position
; FRAME     (0x8069) - Animation frame AND horizontal sub-cell position (0-3)
;
; COLLISION DETECTION NOTES:
; Floor checks only occur when WILLY_Y mod 16 = 0 (cell-aligned).
; If WILLY_Y becomes misaligned, Willy may fall through floors!
;
; ============================================================================
; BUILD INSTRUCTIONS
; ============================================================================
;
; Assemble:  pasmo --bin mm_modbase.asm mm_sprint.bin mm_sprint.sym
; Verify:    wc -c mm_sprint.bin  (should be 32771)
;            grep MESSINTRO mm_sprint.sym  (should be 0x9D00)
; Create TAP: Use Python script with load_addr=0x7FFD
;
; ============================================================================
...
; ============================================================================
; UNUSED BYTES - Available for mod variables (4 bytes at address 0x8418)
; ============================================================================
; Example usage:
;   VELOCITY_X:  DEFB 0    ; Horizontal velocity for momentum
;   MOD_FLAGS:   DEFB 0    ; Flags for mod features
;   UNUSED_VAR2: DEFB 0    ; Available
;   UNUSED_VAR3: DEFB 0    ; Available
; ============================================================================
UNUSED_VARS:
  DEFM "0000"
...
START:
  ...
  LD A,8                  ; Initialise the number of lives remaining at NOMEN
  LD (NOMEN),A            ;
...
; NXSHEET.
NEWSHT:
  XOR A                   ; Clear the regen table on cavern entry
  LD (REGEN_COUNT),A      ;
...
;
; Draw infinite lives at the bottom of the screen (15 Willy sprites)
LOOP:
  LD HL,20640             ; Set HL to the display file address at which to draw
                          ; the first Willy sprite
  LD B,15                 ; Always draw 15 Willy sprites (infinite lives)
...
LOOP_2:
  CALL REGEN_UPDATE       ; Process regen timers and restore expired tiles
...
TM22:
  ...
  JR NZ,LPDEAD1           ; If not, jump back to update the INK colour in the
                          ...
                          ; effect
; Infinite lives - always restart the cavern
  JP NEWSHT               ; Jump back to reinitialise the current cavern
  NOP                     ; (padding - was life check)
  NOP                     ;
  NOP                     ;
  NOP                     ;
  NOP                     ;
...
DECAIR:
  ...
  SUB 8                   ; (2x faster - was SUB 4)
  LD (CLOCK),A            ;
  JR NC,DECAIR_0          ; Jump if no wrap (A was >= 8)
...
DRAWSHEET:
  ...
  LD (SBMSB+1),A          ; SBMSB (below) to 120
DRAWSHEET_0:
...
; Willy is currently jumping.
  ...
  LD (JUMPING),A          ;
; ============================================================================
; >>> HOOK POINT: After JUMPING increment - Variable jump height <<<
; VAR_JUMP checks if jump key is released during rise - if so, mirrors arc
; ============================================================================
  CALL VAR_JUMP           ; Check for early jump release (variable height)
  LD A,(JUMPING)          ; Re-read JUMPING (may have been modified by VAR_JUMP)
...
MOVEWILLY_6:
  ...
  LD (WILLY_Y),A          ;
; ============================================================================
; >>> USEFUL SUBROUTINE: MOVEWILLY_7 - Update LOCATION from WILLY_Y <<<
; Call this after modifying WILLY_Y to keep LOCATION in sync
; Entry: A = WILLY_Y value (or will read from WILLY_Y)
; Exit: HL = updated LOCATION, also stored at (LOCATION)
; ============================================================================
MOVEWILLY_7:
...
; background tile in its place.
  ...
  LD (HL),A               ; Set the attribute at this location to that of the
                          ; background tile
  CALL REGEN_REGISTER     ; Register this tile for regeneration
...
MOVEWILLY2:
  LD A,(AIRBORNE)         ; Pick up the airborne status indicator from AIRBORNE
  CP 18                   ; Has Willy just landed after falling from too great
                          ; a height? (increased from 12 for longer safe falls)
...
MOVEWILLY2_1:
  ...
  LD E,A                  ;
; Sinclair Joystick 1: key 7 (bit 3) = right, key 6 (bit 4) = left
  LD B,239                ; Read keys 0-9-8-7-6
  IN A,(C)
  RRCA                    ; Rotate right 3 times: bit3->bit0, bit4->bit1
  RRCA
  RRCA
  OR 252                  ; Keep only bits 0 (key 7=right) and 1 (key 6=left)
  AND E                   ; Merge into E
  LD E,A
...
; trying to move left.
; ============================================================================
; >>> HOOK POINT: MOVEWILLY2_2 - Input processing <<<
; To add momentum/inertia: insert "CALL YOUR_ROUTINE" here
; E register contains input bits that can be modified before movement
; ============================================================================
MOVEWILLY2_2:
...
; That is left-right movement taken care of. Now check the jump keys.
  ...
  IN A,(C)                ;
  AND 1                   ; Keep only bit 0 (the '0' key / Sinclair fire)
  CP 1                    ; Is '0' being pressed?
  JR NZ,MOVEWILLY2_5      ; Jump if so
  LD A,(KEMP)             ; Collect the Kempston joystick indicator from KEMP
  OR A                    ; Is the joystick connected?
...
; ============================================================================
; >>> HOOK POINT: MOVEWILLY2_6 - Movement application <<<
; To add air control: insert "CALL YOUR_ROUTINE" here
; Called both from ground movement and during jumps
; ============================================================================
MOVEWILLY2_6:
  CALL MOVEMENT_HANDLER   ; Handle sprint, slide, and air control
  LD A,(DMFLAGS)          ; Pick up Willy's direction and movement flags from
                          ; DMFLAGS
...
MOVEHG_0:
  ...
  LD A,(CLOCK)            ; Pick up the value of the game clock at CLOCK
  AND 8                   ; Move bit 3 (which toggles with SUB 8 clock) to bit
  RRCA                    ; 7 and clear all the other bits
  RRCA                    ;
  RRCA                    ;
  RRCA                    ;
...
WILLYATTRS:
  ...
  CALL WILLYATTR_SAFE     ; Set INK for top-left (no collision check)
  INC HL                  ; Move HL to the next cell to the right
  CALL WILLYATTR_SAFE     ; Set INK for top-right (no collision check)
  ...
  LD C,A                  ; Copy it to C
  ADD HL,DE               ; Move HL down a row and back one cell to the left
  CALL WILLYATTR_SAFE     ; Set INK for bottom-left (no collision check)
  INC HL                  ; Move HL to the next cell to the right
  CALL WILLYATTR_SAFE     ; Set INK for bottom-right (no collision check)
...
; ============================================================================
; This area (originally BASIC source code remnants) is available for new code.
; Original size: 2484 bytes (from 0x934C to 0x9D00)
;
; HOW TO ADD NEW CODE:
; 1. Add your routine ABOVE the FREE_SPACE label
; 2. DECREASE the DEFS value by the number of bytes your code uses
; 3. Rebuild and verify binary is exactly 32771 bytes
;
; EXAMPLE - Adding a 50-byte routine:
;   1. Add your code above FREE_SPACE
;   2. Change DEFS 2484 to DEFS 2434 (2484 - 50 = 2434)
;   3. pasmo mm_modbase.asm test.bin && wc -c test.bin
;      Should output: 32771 test.bin
;
; WARNING: If DEFS is too large, binary will overflow 64KB and pasmo warns:
;   "WARNING: 64KB limit passed inside instruction on line XXXX"
;   If this happens, DECREASE the DEFS value until binary is 32771 bytes.
;
; >>> ADD NEW ROUTINES HERE, ABOVE THE FREE_SPACE LABEL <<<
;
; ============================================================================
; MOVEMENT_HANDLER - Main entry for sprint, slide, and air control
; ============================================================================
; Called from MOVEWILLY2_6. Routes to appropriate handler based on state.
;
MOVEMENT_HANDLER:
  CALL SPRINT_CHECK       ; Check if sprint key held, set SPRINT_ACTIVE

  LD A,(AIRBORNE)
  OR A
  JR Z,MH_GROUND          ; On ground
  CP 1
  JP Z,AIR_CONTROL        ; Jumping - use air control
  RET                     ; Falling - no special handling

MH_GROUND:
  ; On ground - handle sprint and slide
  JP GROUND_SLIDE

; ============================================================================
; SPRINT_CHECK - Detect sprint key (number keys 0-9 or joystick up)
; ============================================================================
SPRINT_CHECK:
  PUSH BC
  LD BC,0F7FEH            ; Keys 1-2-3-4-5
  IN A,(C)
  AND 31
  CP 31
  JR NZ,SC_ACTIVE

  LD BC,0EFFEH            ; Sinclair Joystick 1: key 9 (bit 1) = up = sprint
  IN A,(C)
  BIT 1,A                 ; Test key 9
  JR Z,SC_ACTIVE          ; Key 9 pressed (bit=0) = sprint

  ; Check Kempston joystick up (bit 3)
  LD A,(KEMP)             ; Is joystick connected?
  OR A
  JR Z,SC_INACTIVE        ; No joystick
  LD BC,31                ; Kempston port
  IN A,(C)
  BIT 3,A                 ; Check up direction
  JR NZ,SC_ACTIVE         ; Up pressed = sprint
...
; ============================================================================
; UPDATE_ANIM_FRAME - Update visual animation separately from position
; ============================================================================
; When sprinting, animation updates slower so legs are visible
; When not sprinting, animation matches FRAME directly
;
UPDATE_ANIM_FRAME:
  LD A,(SPRINT_ACTIVE)
  OR A
  JR NZ,UAF_SPRINT
  ; Not sprinting - ANIM_FRAME = FRAME
  LD A,(FRAME)
  LD (ANIM_FRAME),A
  RET

UAF_SPRINT:
  ; Sprinting - update animation every 3rd tick
  LD HL,ANIM_TICK
  INC (HL)
  LD A,(HL)
  AND 3                   ; Every 4th tick (0,1,2,3,0,1,2,3...)
  RET NZ                  ; Not time to update yet
  ; Update ANIM_FRAME based on movement direction
  LD A,(DMFLAGS)
  BIT 1,A                 ; Moving?
  JR Z,UAF_STILL          ; Not moving, keep current frame
  BIT 0,A                 ; Direction: 0=right, 1=left
  JR NZ,UAF_LEFT
  ; Moving right - cycle 0->1->2->3->0
  LD A,(ANIM_FRAME)
  INC A
  AND 3
  LD (ANIM_FRAME),A
...
UAF_LEFT:
  ...
  RET
UAF_STILL:
  ; Standing still while sprinting - sync with FRAME
  LD A,(FRAME)
  LD (ANIM_FRAME),A
  RET

; ============================================================================
; GROUND_SLIDE - Handle sprint speed and sliding on ground
; ============================================================================
; When sprinting + direction: move faster, build slide velocity
; When sprinting + no direction: apply slide (gradual stop)
; When not sprinting: clear slide (instant stop)
;
GROUND_SLIDE:
  LD A,(SPRINT_ACTIVE)
  OR A
  ...
  ; Sprinting - check if direction pressed
  LD A,(DMFLAGS)
  BIT 1,A                 ; Moving?
  JR Z,GS_SLIDE           ; No direction, apply slide

  ; Sprinting + direction - build slide velocity and boost speed
  LD A,4                  ; Max slide velocity
  LD (SLIDE_VEL),A
  LD A,(DMFLAGS)
  AND 1
  LD (SLIDE_DIR),A
  XOR A
  LD (SLIDING),A          ; Clear passive slide flag

  ; Speed boost - every 2nd frame to keep animation visible
  LD HL,ANIM_TICK
  INC (HL)
  LD A,(HL)
  AND 1
  RET Z                   ; Skip boost this frame (animation shows)

  ; Apply boost - adjust FRAME for extra movement
  LD A,(DMFLAGS)
  BIT 0,A
  JR NZ,GS_BOOST_LEFT
  ...
  LD A,(FRAME)
  CP 3
  RET Z                   ; At cell boundary, normal move only
  INC A
  LD (FRAME),A
  RET
...
GS_BOOST_LEFT:
  LD A,(FRAME)
  OR A
  RET Z                   ; At cell boundary
  DEC A
  LD (FRAME),A
  RET

GS_SLIDE:
  ; Sprinting but no direction - apply slide
  LD A,(SLIDE_VEL)
  OR A
  RET Z                   ; No velocity, nothing to do

  ; Decrement velocity
  DEC A
  LD (SLIDE_VEL),A

  ; Apply slide direction to DMFLAGS
  LD A,(SLIDE_DIR)
  OR 2                    ; Set moving bit
  LD (DMFLAGS),A
  RET
...
GS_NO_SPRINT:
  ; Not sprinting - continue slide until velocity depletes
  LD A,(SLIDE_VEL)
  OR A
  JR NZ,GS_APPLY_SLIDE
  ; Velocity depleted - clear sliding flag
  LD (SLIDING),A          ; A is 0 here
  RET
GS_APPLY_SLIDE:
  ; Apply slide (same as GS_SLIDE but don't allow direction input)
  DEC A
  LD (SLIDE_VEL),A
  ; Freeze animation during involuntary slide - don't set moving bit
  ; Just apply direction without moving flag so FRAME doesn't update
  LD A,(SLIDE_DIR)
  OR 2                    ; Set moving bit for position update
  LD (DMFLAGS),A
  LD A,1                  ; Signal we're in passive slide mode
  LD (SLIDING),A
  RET

; Sprint state variables
SPRINT_ACTIVE:
  DEFB 0                  ; 1 if sprint key held this frame
SLIDE_VEL:
  DEFB 0                  ; Ground slide velocity (0-4)
SLIDE_DIR:
  DEFB 0                  ; Direction of slide (0=right, 1=left)
SPRINT_JUMP:
  DEFB 0                  ; 1 if jump started while sprinting
SLIDING:
  DEFB 0                  ; 1 if in passive slide (sprint released, still moving)
ANIM_TICK:
  DEFB 0                  ; Animation tick counter (for sprint animation)
ANIM_FRAME:
  DEFB 0                  ; Visual animation frame (0-3), updates slower when sprinting

; ============================================================================
; AIR_CONTROL - Allow direction changes while jumping
; ============================================================================
; Called from MOVEMENT_HANDLER. Provides momentum-based air control:
; - No input: continues moving in initial jump direction (momentum)
; - Same direction as momentum: boosted movement (extra speed every 3rd frame)
; - Opposite direction: reduced movement (every other frame)
;
AIR_CONTROL:
  LD A,(AIRBORNE)
  CP 1                    ; Only act if jumping (AIRBORNE=1)
  JP NZ,AC_NOT_JUMPING    ; Not jumping, clear state and return

  PUSH BC

  ; Check if first frame of jump (JUMPING 0 or 1)
  LD A,(JUMPING)
  CP 2
  JR NC,AC_READ_INPUT     ; Skip if JUMPING >= 2

  ; First frame - reset counters
  XOR A
  LD (MOMENTUM_CTR),A
  ; Capture sprint state for enhanced momentum
  LD A,(SPRINT_ACTIVE)
  LD (SPRINT_JUMP),A
  ; Note: MOMENTUM_DIR and JUMP_FACING set after reading input below

AC_READ_INPUT:
  ; Read keyboard for left/right input
  LD BC,57342             ; Read keys P-O-I-U-Y
  IN A,(C)
  AND 31
  LD E,A
...
AC_PROCESS:
  ; E has input: bit 1=0 means left pressed, bit 0=0 means right pressed
  ; Determine input direction: D = 0 (none), 1 (left), 2 (right)
  LD D,0                  ; Default: no input
  LD A,E
  AND 2
  JR NZ,AC_CHK_R_INPUT
  LD D,1                  ; Left pressed
  JR AC_COMPARE
AC_CHK_R_INPUT:
  LD A,E
  AND 1
  JR NZ,AC_COMPARE
  LD D,2                  ; Right pressed

AC_COMPARE:
  ; D = input (0=none, 1=left, 2=right)
  ; On first frames (JUMPING 0 or 1), capture momentum and facing
  LD A,(JUMPING)
  CP 2
  JR NC,AC_CHK_MOMENTUM   ; Skip if JUMPING >= 2 (not first frame)
  ; First frame - set MOMENTUM_DIR and JUMP_FACING based on input D
  LD A,D
  CP 0
  JR Z,AC_NO_MOM_CAPTURE  ; No input = no momentum (standing jump)
  CP 1
  JR Z,AC_MOM_LEFT_CAP
  ; Right: set facing=0 (right), momentum=2
  XOR A
  LD (JUMP_FACING),A      ; Face right
  LD A,2                  ; bit1=1 (moving), bit0=0 (right)
  JR AC_SAVE_MOM
AC_MOM_LEFT_CAP:
  ; Left: set facing=1 (left), momentum=3
  LD A,1
  LD (JUMP_FACING),A      ; Face left
  LD A,3                  ; bit1=1 (moving), bit0=1 (left)
AC_SAVE_MOM:
  LD (MOMENTUM_DIR),A
  JR AC_CHK_MOMENTUM
AC_NO_MOM_CAPTURE:
  ; Standing jump - keep current facing direction
  LD A,(DMFLAGS)
  AND 1
  LD (JUMP_FACING),A      ; Keep current facing
  XOR A                   ; No momentum
  LD (MOMENTUM_DIR),A

AC_CHK_MOMENTUM:
  ; Compare with momentum direction
  LD A,(MOMENTUM_DIR)
  BIT 1,A                 ; Was Willy moving at jump start?
  JR Z,AC_NO_MOMENTUM     ; No momentum, just apply input directly

  ; Has momentum - check input against it
  ; MOMENTUM_DIR bit 0: 1=left, 0=right
  BIT 0,A
  JR Z,AC_MOM_RIGHT
  ...
  LD A,D
  CP 0
  JR Z,AC_USE_MOMENTUM    ; No input, use momentum
  CP 1
  JR Z,AC_SAME_LEFT       ; Same direction (left), boost
  JR AC_OPP_RIGHT         ; Opposite (right pressed, momentum left)
...
AC_MOM_RIGHT:
  ...
  JR Z,AC_USE_MOMENTUM    ; No input, use momentum
  CP 2
  JR Z,AC_SAME_RIGHT      ; Same direction (right), boost
  JR AC_OPP_LEFT          ; Opposite (left pressed, momentum right)

AC_NO_MOMENTUM:
  ; No momentum at jump start - apply input directly
  LD A,D
  CP 1
  JR Z,AC_SET_LEFT
  CP 2
  JR Z,AC_SET_RIGHT
  ; No input and no momentum - don't move
  POP BC
  RET

AC_USE_MOMENTUM:
  ; No input - apply momentum direction but keep jump facing
  LD A,(MOMENTUM_DIR)
  AND 2                   ; Keep only moving bit
  LD B,A
  LD A,(JUMP_FACING)      ; Get original facing direction
  OR B                    ; Combine facing with movement
  LD (DMFLAGS),A
  POP BC
  RET

AC_SAME_LEFT:
  ; Pressing same direction as momentum (left) - boost every 3rd frame
  LD A,(JUMP_FACING)      ; Keep original facing
  OR 2                    ; Add moving bit
  LD (DMFLAGS),A
  CALL AC_BOOST
  POP BC
...
AC_SAME_RIGHT:
  ; Pressing same direction as momentum (right) - boost every 3rd frame
  LD A,(JUMP_FACING)      ; Keep original facing
  OR 2                    ; Add moving bit
  LD (DMFLAGS),A
  CALL AC_BOOST
...
AC_OPP_LEFT:
  ; Pressing opposite - skip more frames if sprint jump
  CALL AC_OPP_CHECK
  JR Z,AC_SKIP_MOVE       ; Skip this frame
AC_SET_LEFT:
  LD A,(JUMP_FACING)      ; Keep original facing
  OR 2                    ; Add moving bit
  LD (DMFLAGS),A
...
AC_OPP_CHECK:
  ; Returns Z if should skip move. Sprint: move 1 in 3, Normal: move 1 in 2
  LD A,(SPRINT_JUMP)
  OR A
  JR Z,AC_OPP_NORMAL
  ; Sprint jump - only move every 3rd frame
  LD A,(MOMENTUM_CTR)
  INC A
  CP 3
  JR C,AC_OPP_SAVE
...
AC_OPP_SAVE:
  LD (MOMENTUM_CTR),A
  CP 1                    ; Only move when counter=1 (Z if skip)
  RET
AC_OPP_NORMAL:
  ; Normal - move every other frame
  LD A,(MOMENTUM_CTR)
  INC A
  AND 1
  LD (MOMENTUM_CTR),A
  RET                     ; Z flag set if counter=0 (skip)

AC_SKIP_MOVE:
  ; Don't move this frame (reduced movement for opposite direction)
  LD A,(JUMP_FACING)      ; Keep original facing, no moving bit
  LD (DMFLAGS),A
  POP BC
  RET

AC_BOOST:
  ; Boost movement - every 2nd frame if sprint jump, every 3rd otherwise
  LD A,(MOMENTUM_CTR)
  INC A
  LD B,A                  ; Save incremented value
  LD A,(SPRINT_JUMP)
  OR A
  LD A,B                  ; Restore counter
  JR Z,AC_BOOST_NORMAL
  ; Sprint jump - boost every 2nd frame
  CP 2
  JR C,AC_BOOST_SAVE
  XOR A
  JR AC_BOOST_SAVE
AC_BOOST_NORMAL:
  ; Normal jump - boost every 3rd frame
  CP 3
  JR C,AC_BOOST_SAVE
  XOR A                   ; Reset to 0
AC_BOOST_SAVE:
  LD (MOMENTUM_CTR),A
  OR A
  JR Z,AC_DO_BOOST        ; Counter is 0, do boost
  ; Counter != 0, but sprint jumps always boost
  LD A,(SPRINT_JUMP)
  OR A
  RET Z                   ; Not sprint jump, skip this frame
AC_DO_BOOST:
  ; Boost frame - adjust FRAME for extra movement
  LD A,(DMFLAGS)
  BIT 0,A                 ; Direction: 1=left, 0=right
  JR NZ,AC_BOOST_LEFT
  ; Boost right - increment FRAME
  LD A,(FRAME)
  CP 3
  RET Z                   ; At max, can't boost (would need cell change)
  INC A
  LD (FRAME),A
  RET
AC_BOOST_LEFT:
  ; Boost left - decrement FRAME
  LD A,(FRAME)
  OR A
  RET Z                   ; At min, can't boost
  DEC A
  LD (FRAME),A
  RET

AC_NOT_JUMPING:
  ; Not jumping - clear momentum state
  XOR A
  LD (MOMENTUM_DIR),A
  RET
...
MOMENTUM_DIR:
  DEFB 0                  ; Direction at jump start (DMFLAGS format)
MOMENTUM_CTR:
  DEFB 0                  ; Counter for boost/reduce timing
JUMP_FACING:
  DEFB 0                  ; Facing direction at jump start (0=right, 1=left)

; ============================================================================
; VAR_JUMP - Variable jump height with rising inertia
; ============================================================================
; When jump key is released during rise, continues rising for ~2 more frames
; (inertia) before mirroring to descent. Creates Mario-style momentum feel.
; Only applies when Willy is moving (bit 1 of DMFLAGS set).
;
; Uses JUMP_RELEASE to track when key was released:
;   0 = key held or not tracking
;   1-7 = JUMPING value when key was released
;
; The cut happens at MIN(release_point + 2, 7), then mirrors from there.
; ============================================================================

VAR_JUMP:
  LD A,(JUMPING)
  CP 8
  JR NC,VJ_RESET          ; Past apex, clear state and return

  LD C,A                  ; C = current JUMPING

  ; Check if we have a pending release point
  LD A,(JUMP_RELEASE)
  OR A
  JR NZ,VJ_HAS_RELEASE

  ; No pending release - check if key is currently held
  PUSH BC

  LD BC,32510             ; SHIFT-Z-X-C-V and B-N-M-SS-SPACE
  IN A,(C)
  AND 31
  CP 31
  JR NZ,VJ_HELD           ; Key held

  LD B,239                ; 0-9-8-7-6 row
  IN A,(C)
  AND 1                   ; Keep only bit 0 (key 0 / Sinclair fire)
  CP 1
  JR NZ,VJ_HELD           ; Key 0 held

  LD A,(KEMP)
  OR A
  JR Z,VJ_KEY_RELEASED    ; No joystick, key not held

  LD BC,31
  IN A,(C)
  BIT 4,A
  JR NZ,VJ_HELD           ; Fire held

VJ_KEY_RELEASED:
  ; Key released - store release point for inertia
  POP BC                  ; Restore C = current JUMPING
  LD A,C
  LD (JUMP_RELEASE),A
  RET                     ; Continue rising (inertia begins)

VJ_HELD:
  ; Key held or not moving - clear any pending release
  XOR A
  LD (JUMP_RELEASE),A
  POP BC
  RET
...
VJ_HAS_RELEASE:
  ; A = release point, C = current JUMPING
  ; First check if this is a new jump (current < release means new jump)
  LD B,A                  ; B = release point
  LD A,C                  ; A = current
  CP B                    ; current - release
  JR C,VJ_RESET           ; current < release: new jump, clear state

  ; Calculate cut point = MIN(release + 2, 7)
  LD A,B
  ADD A,2
  CP 8
  JR C,VJ_CUT_POINT_OK
  LD A,7                  ; Cap at 7 (just before apex)
VJ_CUT_POINT_OK:
  ; A = cut point, C = current JUMPING
  CP C                    ; cut_point - current
  JR C,VJ_CUT             ; cut_point < current: time to cut
  JR Z,VJ_CUT             ; cut_point == current: time to cut
  RET                     ; cut_point > current: continue rising

VJ_CUT:
  ; Mirror from current position: JUMPING = 18 - current
  LD A,18
  SUB C
  LD (JUMPING),A
...
; State variable for tracking jump key release
JUMP_RELEASE:
  DEFB 0

; Safe version of WILLYATTR - sets INK but skips nasty collision check
; Used for forgiving hitbox on corners
WILLYATTR_SAFE:
  LD A,(BACKGROUND)       ; Pick up the attribute byte of the background tile
  CP (HL)                 ; Does this cell contain a background tile?
  RET NZ                  ; Return if not background (don't change INK)
  LD A,C                  ; Check if we should set INK
  AND 15                  ;
  RET Z                   ; Return if we should retain current INK
  LD A,(BACKGROUND)       ; Pick up background attribute
  OR 7                    ; Set INK to white
  LD (HL),A               ; Set the attribute byte
  RET

; ============================================================================
; REGEN_REGISTER - Register a destroyed crumbling tile for regeneration
; ============================================================================
; Called from CRUMBLE when a tile is fully destroyed.
; HL = address of the tile in the 24064 attribute buffer.
;
REGEN_REGISTER:
  PUSH AF
  PUSH DE
  ...
  LD A,(REGEN_COUNT)
  CP 16                   ; Table full?
  JR NC,REGEN_REG_DONE    ; Yes, skip
  ; Calculate slot address: REGEN_TABLE + count * 4
  LD E,A
  SLA E
  SLA E                   ; E = count * 4
  LD D,0
  ...
  POP DE                  ; DE = buffer address (was HL)
  ; Store offset: low byte and high byte relative to 24064 (0x5E00)
  LD A,E                  ; Low byte of buffer address
  LD (HL),A               ; offset_lo
  INC HL
  LD A,D
  SUB 94                  ; 24064 = 0x5E00, high byte - 0x5E = 0 or 1
  LD (HL),A               ; offset_hi
  INC HL
  LD A,250                ; Timer = 250 (0x00FA), low byte = 250
  LD (HL),A               ; timer_lo
  INC HL
  XOR A                   ; Timer high byte = 0
  LD (HL),A               ; timer_hi
  ; Increment count
  LD A,(REGEN_COUNT)
  INC A
...
; ============================================================================
; REGEN_UPDATE - Tick all regen timers, restore expired tiles
; ============================================================================
; Called each frame from LOOP_2 before buffer copies.
;
REGEN_UPDATE:
  LD A,(REGEN_COUNT)
  OR A
  RET Z                   ; No active slots
  LD B,A                  ; B = slot count (loop counter)
  LD IX,REGEN_TABLE
REGEN_UPD_LOOP:
  ; Decrement 16-bit timer at IX+2, IX+3
  LD L,(IX+2)
  LD H,(IX+3)
  DEC HL
  LD (IX+2),L
  ...
  LD A,L
  OR H
  JR NZ,REGEN_UPD_NEXT
  ; Timer expired - restore tile and remove slot
  PUSH BC
  CALL REGEN_RESTORE
  CALL REGEN_REMOVE_SLOT
  POP BC
  JR REGEN_UPD_CONT       ; Slot was swapped, don't advance IX
REGEN_UPD_NEXT:
  LD DE,4
  ADD IX,DE
REGEN_UPD_CONT:
...
; ============================================================================
; REGEN_RESTORE - Restore a crumbling tile's attribute and pixels
; ============================================================================
; IX points to the regen slot being restored.
;
REGEN_RESTORE:
  ; Restore attribute byte in 24064 buffer
  LD L,(IX+0)             ; offset_lo
  LD A,(IX+1)             ; offset_hi
  ADD A,94                ; 94 = 0x5E, base high byte of 24064
  LD H,A                  ; HL = address in 24064 buffer
  LD A,(CRUMBLING)        ; Attribute byte for crumbling tile
  LD (HL),A               ; Restore attribute
  ; Restore pixel data in 28672 screen buffer
  ; Top half (offset_hi=0): D=112 (0x70), Bottom half (offset_hi=1): D=120 (0x78)
  LD A,(IX+1)
  SLA A
  SLA A
  SLA A                   ; 0->0, 1->8
  ADD A,112               ; 112 or 120
  LD D,A
  LD E,(IX+0)             ; E = offset_lo
  ; Copy 8 bytes from CRUMBLING+1 to screen buffer
  LD HL,CRUMBLING+1       ; Tile graphic data (8 bytes after attribute)
  LD B,8
REGEN_COPY:
  LD A,(HL)
  LD (DE),A
...
; ============================================================================
; REGEN_REMOVE_SLOT - Remove a slot by swapping with last active slot
; ============================================================================
; IX points to the slot to remove. Decrements REGEN_COUNT.
;
REGEN_REMOVE_SLOT:
  LD A,(REGEN_COUNT)
  DEC A
  LD (REGEN_COUNT),A
  ; Calculate address of last slot: REGEN_TABLE + A * 4
  LD L,A
  SLA L
  SLA L                   ; L = (new count) * 4
  LD H,0
  LD DE,REGEN_TABLE
  ADD HL,DE               ; HL = address of last slot
  ; Check if IX == HL (already the last slot)
  PUSH IX
  POP DE                  ; DE = current slot address
  LD A,D
  CP H
  JR NZ,REGEN_DO_SWAP
  LD A,E
  CP L
  RET Z                   ; Same slot, nothing to do
REGEN_DO_SWAP:
  ; Copy 4 bytes from HL (last) to DE (current)
  LD A,(HL)
  LD (DE),A
  INC HL
  INC DE
...
; Regen state data
REGEN_COUNT:
  DEFB 0                  ; Number of active regen slots
REGEN_TABLE:
  DEFS 64                 ; 16 slots x 4 bytes (offset_lo, offset_hi, timer_lo, timer_hi)

; ============================================================================
FREE_SPACE:
  DEFS 1507               ; Padding to maintain exact binary size
